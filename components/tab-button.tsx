...
export function TabButton({ active, label, icon, badge, onPress }: Props) {
  return (
    ...
    >
      ...
      {badge !== undefined && (
        <View className={`rounded-full px-1.5 py-0.5 ${active ? "bg-zinc-100 dark:bg-zinc-700" : "bg-zinc-200 dark:bg-zinc-800"}`}>
          <Text className={`text-[10px] font-bold ${active ? "text-zinc-900 dark:text-white" : "text-zinc-500"}`}>
            {badge}
          </Text>
        </View>
      )}
